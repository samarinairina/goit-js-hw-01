...
// const getSortedUniqueSkills = users => users.filter(({ skills }) => skills).reduce((allskills, skiil) => {
//   allskills.push(...users.skills);

//   return allskills;
// }, []).sort();

//const getSortedUniqueSkills = users => users.reduce((acc, skill) => acc.concat(skills), []).filter(onlyUnique).sort((a, b) => a.skills-b.skills);
const getSortedUniqueSkills = users => users.map(function({skills}, i) {
  return skills}).reduce(function(allskills, skills) { return  allskills.concat(...skills)}, [])
